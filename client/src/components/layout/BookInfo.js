...
function BookInfo(props) {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/books/${props.match.params.id}/user`)
      .then((res) => {
        setBookOwner({
  ...
  const deleteBook = (bookId) => {
    axios
      .delete("http://localhost:4000/books/" + bookId)
      .then((res) => {
    ...
    <div className="bookinfo-wrapper">
      <Row>
        <Col className="bookinfo-image-wrapper" md={6} xs={12}>
          <img
            src={renderImage(oldBook.imageBuffer, oldBook.imageType)}
            alt={oldBook.bookName}
          />
          {!isForbidden ? (
            <div>
              <Tooltip title="Edit record" placement="bottom">
                <Link to={`/books/edit/${props.match.params.id}`}>
        ...
        <Col md={6} xs={12}>
          <div className="book-owner-wrapper">
            ...
            <Link to={`/users/profile/${bookOwner.ownerId}`}>
              <Button className="uploaded-by-link" variant="link">
                {bookOwner.ownerName}
              </Button>
            </Link>
          </div>
          ...
          <hr />
          <p>
            <EditAttributesIcon /> {oldBook.author}
          </p>
          <p>
            <MenuBookIcon /> {oldBook.genre}
          </p>
          <hr />
          {/* "Read More" if text longer than 530 */}
          <p className="book-des">
            {isMore === "more"
              ? oldBook.bookDes.substring(0, 530)
              : oldBook.bookDes}
          </p>
          {oldBook.bookDes.length > 530 ? (
